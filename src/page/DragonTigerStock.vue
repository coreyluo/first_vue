...
<template>
    <div class="layout">
        <Sider :style="{position: 'fixed', height: '100vh', left: 0, overflow: 'auto'}">
            <Menu active-name="1-28" theme="dark" width="auto" :open-names="['1']" @on-select="routeTo">
                <Submenu name="1">
                    <template slot="title">
        ...
        <Layout :style="{marginLeft: '200px'}">
            ...
            <template>
                <div>
                  <Button type="error" size="small">龙虎榜</Button>
                </div>
            </template>
...
<script>
    export default {
        created () {
            this.$api.get('singular/dragonAndTiger/list', null, r => {
                var infos = r.data;
                infos.forEach(item => {
                  if(item.dragonTigerType==0){
                    item.dragonTigerTypeStr = "上证";
                  }
                  if(item.dragonTigerType==1){
                    item.dragonTigerTypeStr = "深圳主板";
                  }
                  if(item.dragonTigerType==2){
                    item.dragonTigerTypeStr = "创业板";
                  }
                  if(item.dragonTigerType==3){
                    item.dragonTigerTypeStr = "中小板";
                  }
