...
<template>
    <div class="layout">
        <Sider :style="{position: 'fixed', height: '100vh', left: 0, overflow: 'auto'}">
            <Menu active-name="1-26" theme="dark" width="auto" :open-names="['1']" @on-select="routeTo">
                <Submenu name="1">
                    <template slot="title">
...
<script>
    export default {
        created () {
            this.$api.get('singular/monsterIncrease/dataList', null, r => {
              var infos = r.data;
              infos.forEach(item => {
        ...
        methods: {
          addAll(endStatus) {
            this.$api.get('singular/monsterIncrease/addAll', {}, r => {
            })
            location.reload()
          },
          addOne(id) {
            this.$api.get('singular/monsterIncrease/addOne', {id:id}, r => {
            })
            location.reload()
          },
          cancelOne(id) {
            this.$api.get('singular/monsterIncrease/cancelOne', {id:id}, r => {
            })
            location.reload()
          }
        }
